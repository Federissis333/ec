...
module.exports = {
    name: "embed",
    description: "[💻 Administração] Crie uma embed personalizada.",
    type: 1,
    options: [],
    permissions: {},
    run: async (client, interaction) => {
        ...
        await VerifyButtons();
        interaction.reply({ embeds: [embed], components: [row1, row2, row3] }).then(async (msg) => {
            ...
            collector.on('collect', async (i) => {
                ...
                if (i.user.id === interaction.user.id) {
                    ...
                    if (i.customId === "title") {
                        ...
                        i.deferUpdate();
                        await VerifyEmbed()
                        await interaction.channel.send({ embeds: [newEmbed], components: [] }).catch((err) => { })
