...
module.exports = {
    config: {
        name: "instagram",
        aliases: ["insta"],
        description: "Configure o sistema de instagram.",
        category: "ADM",
        usage: null,
    },
    permissions: null,
    ...
    run: async (client, message, args) => {
        ...
        message.reply({ embeds: [embed], components: [row] }).then(async (msg) => {
            ...
            collector.on('collect', async (i) => {
                ...
                if (i.user.id === message.author.id) {
                    if (i.customId === "estado-ativar") {
                        ...
                        await client.guilddb.updateOne({ IDs: message.guild.id }, { $set: { "insta.status": true } })
                        ...
                        await client.guilddb.updateOne({ IDs: message.guild.id }, { $set: { "insta.status": false } })
                        ...
                        collector.on('collect', async (m) => {
                            ...
                            await client.guilddb.updateOne({ IDs: message.guild.id }, { $set: { "insta.canal": canal.id } })
                        ...
                        await client.guilddb.updateOne({ IDs: message.guild.id }, { $set: { "insta.canal": "null" } })
                        i.deferUpdate();
            ...
            collector.on('end', async (collected) => {
                ...
                reset.setDisabled(true)
                msg.edit({ components: [new ActionRowBuilder().addComponents(estado).addComponents(canalb).addComponents(reset)] })
            });
